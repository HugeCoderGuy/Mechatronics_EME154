  ...
  #define BRAKE 8
  #define MAX_PWM 255
  #define MIN_PWM 40 // this one depends on the dead zone of the motor input voltage
  #define PPR 1080   // 1080 is ideally, but there may exists some offset from your observation and sensor noise 
  
  // interpolated P-control
  #define KP 0.9  // P control parameter
  #define KI 0.01  // Integral  Parameter
  #define TARGET_DIST 50 // pulses
  #define DIS2GO 1.5
  int dist_moved = 0;
  int controlLoopRate = 95; //unchanged
  int distanceToGo;    
  double integral = 0;  //used for I control
  double positionError=0;
  double prevPositionError = 0; //used for I control
  int PWM_value;
  ...
  ISR(TIMER1_COMPA_vect) {
    ...
    if (abs(distanceToGo - encoderPos) < TARGET_DIST){
      ...
      integral += KI*.5*(positionError+ prevPositionError)/controlLoopRate; //Trapazoidal rule to find integral
    ...
    // positionError = distanceToGo - encoderPos;
    PWM_value = (int)(KP*(float)positionError + integral); //PI control
    //Serial.print(PWM_value); Serial.print("  ");
    //Serial.println(integral);
    //calculate integral
    // update the last_encoder reading and position error
  ...
  bool flagCcw = true; //used to prevent aditional spin of dial when dial = combo
  ...
  int change = combo-dial;  //amount of ticks motor needs to move (can be + or -)
  if(dial > combo){    
    driveTicks(change); //when dial>combo, change is - and goes CCW
    dial = combo;  //reset dial
    flagCcw = false;  //Prevents adiditional spin
  }
  if(dial<combo) {
    int completeTicks = - dial;
    driveTicks(completeTicks);    //bring the dial to 0 by going CCW
    dial = 0;
    int completeTicks2 = combo - dial_ticks;
    driveTicks(completeTicks2);   //bring dial to combo CCW
    dial = combo;
  }
  else if (flagCcw){  //full rotation of moveCcwToDialNo() is called with dial input
    drivePulses(-PPR);
    flagCcw=true;
  }
}
  
  bool flagCw = true;  //same as flagCcw but for clockwise
  /* move CW to a dial position */
  // input: dial position combo it will go to;
  // output: the motor will rotate to that number CW;
  // calculate the incremental ticks it needs to go and use function 'driveTicks(ticks)' to implement
  void moveCwToDialNO(int combo)  {
  int change = combo-dial; 
  if(dial > combo){
    int completeTicks = dial_ticks - dial;
    driveTicks(completeTicks);  //Move the dial to 0 by going CW
    dial = 0; 
    driveTicks(combo);
    dial = combo;  //complete dial movement in CW direction
    flagCw = false;  //prevent additional spin
  }
  if(dial<combo) {
    driveTicks(change);  //change is + with dial<combo and driveTicks() can by called with change
    dial = combo;
  }
  else if (flagCw){
    drivePulses(PPR);
  ...
  void setAutomaticMenu(){
    ...
    str1 = "AUTOMATIC OPORATION MENU";
    str2 = " ";
    str3 = "Please input your 3";
    str4 = "combo values ";
    str5 = " ";
    str1.toCharArray(menuTitle,25);
    str2.toCharArray(action1, 25);
  ...
  void ACSfunction(){
    ...
    String str;
    while(OMD ==2) {
      switch(task){  //switch command helps keep flow
      // task 0
      case 0:
        // prompt to enter the first combination number combo1 and update machine message
        Serial.print("Please input first combo value: ");
        // switch to task 1
        task=1;
        break;
      // task 1
      case 1:
        ...
        if(combo1 <= dial_ticks && combo1 >=0) {
          Serial.println(combo1);
          Serial.print("Please input second combo value: ");
          task = 2;
        }
        else {  //user enters wrong value
          Serial.print("\nValue ");Serial.print(combo1);Serial.print(" is too large or too small");
          Serial.println("\nReinput value between 0 and 60");
          Serial.print("Input 1 = ");
          task = 1;  //redo until correct input
        }
        break;
      ...
      case 2:
        ...
        if(combo2 <= dial_ticks && combo2 >=0) {
          Serial.println(combo2);
          Serial.print("Please input third combo value: ");
          task = 3;
        }
        else {  //user enters wrong value
          Serial.print("\nValue ");Serial.print(combo2);Serial.print(" is too large or too small");
          Serial.println("\nRenput value between 0 and 60");
          Serial.print("Input 2 = ");

          task = 2; //redo until correct input
        }
        break;
        // check the number input if correct or not, reject the number over the range
        // if it over the range, prompt the user to input it again
      ...
      case 3:
        ...
        if(combo3 <= dial_ticks && combo3 >=0) {
          Serial.println(combo3);
          Serial.print("Your combo is: ");
          Serial.print(combo1); Serial.print(", ");
          Serial.print(combo2); Serial.print(", ");
          Serial.println(combo3); 
          task = 4;
        }
        else{  //user enters wrong value
          Serial.print("\nValue ");Serial.print(combo3);Serial.print(" is too large or too small");
          Serial.println("\nRenput value between 0 and 60");
          Serial.print("Input 3 = ");

          task = 3;  //redo until correct input
        }
        break;
        // check the number input if correct or not, reject the number over the range
        // if it over the range, prompt the user to input it again
      ...
      case 4:  
        ...
        Serial.println("Press # to open your lock!");
        ...
        Serial.print("Press # to return to the main menu");
        key = customKeypad.getKey();
        while(key!='#'){
          key = customKeypad.getKey();
        };
